...
from src.model.model_inverse_domain import *

# import the example file
from examples.cd2d_inverse_circle_example import *

#import physics for custom loss function
from src.physics.cd2d_inverse_domain import *
...
if __name__ == "__main__":
    ...
    points, sensor_values = datahandler.get_sensor_data(exact_solution, num_sensor_points=i_num_sensor_points, mesh_type=i_mesh_generation_method, file_name=i_sensor_data_file)
    
    


    model = DenseModel_Inverse_Domain(layer_dims = i_model_architecture, learning_rate_dict = i_learning_rate_dict, \
                       params_dict = params_dict, \
                       loss_function = pde_loss_cd2d_inverse_domain, input_tensors_list = [datahandler.x_pde_list, train_dirichlet_input, train_dirichlet_output], \
    ...
    if i_mesh_generation_method == "internal":
        ...
        plot_contour(x = X, y = Y, z = Y_Exact_Matrix, output_path = i_output_path, filename= f"exact_solution", title = "Exact Solution")
    

    # obtain the target inverse parameters
    target_inverse_params_dict = get_inverse_params_actual_dict(test_points[:,0], test_points[:,1]) # Obtain the target inverse parameters at the test points
    ...
    for epoch in range(num_epochs):   
        ...
        if (epoch+1) % i_update_console_output == 0 or epoch == num_epochs-1:
            ...
            l2_error_inverse, linf_error_inverse, l2_error_relative_inverse, linf_error_relative_inverse, \
                l1_error_inverse, l1_error_relative_inverse = compute_errors_combined(actual_epsilon, inverse_pred)
            ...
            console.print(f"Variational Losses || Pde Loss : [red]{loss_pde:.3e}[/red] Dirichlet Loss : [red]{loss_dirichlet:.3e}[/red] Sensor Loss : [red]{sensor_loss:.3e}[/red] Total Loss : [red]{total_loss:.3e}[/red]")
            ...
            console.print(f"InvPrm Test Losses        || L1 Error_inverse : {l1_error_inverse:.3e}", end=" ")
            ...
            if i_mesh_generation_method == "internal":
                ...
                plot_contour(x = X, y = Y, z = inverse_pred, output_path = i_output_path, filename= f"inverse_prediction_{epoch+1}", title = "Inverse Parameter Prediction")
                # plot the inverse parameter error
                plot_contour(x = X, y = Y, z = inverse_error, output_path = i_output_path, filename= f"inverse_error_{epoch+1}", title = "Inverse Parameter Error")

            elif i_mesh_generation_method == "external":
                solution_array = np.c_[y_pred, y_exact, np.abs(y_exact - y_pred), inverse_pred, actual_epsilon, np.abs(actual_epsilon - inverse_pred)]
                error = np.abs(y_exact - y_pred)
                domain.write_vtk(solution_array, output_path = i_output_path, filename= f"prediction_{epoch+1}.vtk", data_names = ["Sol","Exact", "Error","Inv","ExactInv","InvError"] )
    ...
    np.savetxt(str(Path(i_output_path) / "inverse_error.txt"), np.abs(actual_epsilon - inverse_pred))
    ...
    if i_use_wandb:
        ...
        if i_mesh_generation_method == "internal":
            # save the final prediction and error
            wandb.log({"prediction": wandb.Image(i_output_path + f"/prediction_{epoch+1}.png")})
            wandb.log({"error": wandb.Image(i_output_path + f"/error_{epoch+1}.png")})
            wandb.log({"inverse_prediction": wandb.Image(i_output_path + f"/inverse_prediction_{epoch+1}.png")})
            wandb.log({"inverse_error": wandb.Image(i_output_path + f"/inverse_error_{epoch+1}.png")})
        elif i_mesh_generation_method == "external":
            wandb.log({"prediction": wandb.Image(i_output_path + f"/prediction_{epoch+1}.png")})
            wandb.log({"error": wandb.Image(i_output_path + f"/Error_{epoch+1}.png")})
            wandb.log({"inverse_prediction": wandb.Image(i_output_path + f"/InvError_{epoch+1}.png")})
            wandb.log({"inverse_error": wandb.Image(i_output_path + f"/Inv_{epoch+1}.png")})
            wandb.log({"exact": wandb.Image(i_output_path + f"/ExactInv_{epoch+1}.png")})
            #log a vtk file
            wandb.save(str(Path(i_output_path) / f"prediction_{epoch+1}.vtk"))
        ...
        wandb.save(str(Path(i_output_path) / "inverse_prediction.txt"))
        wandb.save(str(Path(i_output_path) / "inverse_error.txt"))
