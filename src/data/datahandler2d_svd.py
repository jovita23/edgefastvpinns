...
class DataHandler2DSVD():
    ...
    def __init__(self, fespace, domain, dtype):
        ...
        self.domain = domain
        self.shape_val_U_mat_list = []
        self.shape_val_VT_mat_list = []
        self.grad_x_U_mat_list = []
        self.grad_x_VT_mat_list = []
        self.grad_y_U_mat_list = []
        self.grad_y_VT_mat_list = []
        self.x_pde_list = []
        ...
        for cell_index in range(self.fespace.n_cells):
            shape_val_mat_u = tf.constant(self.fespace.get_shape_function_U(cell_index), dtype=self.dtype)
            shape_val_mat_vt = tf.constant(self.fespace.get_shape_function_VT(cell_index), dtype=self.dtype)

            grad_x_mat_u = tf.constant(self.fespace.get_shape_function_grad_x_U(cell_index), dtype=self.dtype)
            grad_x_mat_vt = tf.constant(self.fespace.get_shape_function_grad_x_VT(cell_index), dtype=self.dtype)

            grad_y_mat_u = tf.constant(self.fespace.get_shape_function_grad_y_U(cell_index), dtype=self.dtype)
            grad_y_mat_vt = tf.constant(self.fespace.get_shape_function_grad_y_VT(cell_index), dtype=self.dtype)
            ...
            self.shape_val_U_mat_list.append(shape_val_mat_u)
            self.shape_val_VT_mat_list.append(shape_val_mat_vt)
            self.grad_x_U_mat_list.append(grad_x_mat_u)
            self.grad_x_VT_mat_list.append(grad_x_mat_vt)
            self.grad_y_U_mat_list.append(grad_y_mat_u)
            self.grad_y_VT_mat_list.append(grad_y_mat_vt)
        ...
        self.shape_val_mat_U_list = tf.stack(self.shape_val_U_mat_list, axis=0)
        self.shape_val_mat_VT_list = tf.stack(self.shape_val_VT_mat_list, axis=0)

        # store it in a single list
        self.shape_val_mat_svd_list = [self.shape_val_mat_U_list, self.shape_val_mat_VT_list]

        self.grad_x_mat_U_list = tf.stack(self.grad_x_U_mat_list, axis=0)
        self.grad_x_mat_VT_list = tf.stack(self.grad_x_VT_mat_list, axis=0)

        self.grad_x_mat_svd_list = [self.grad_x_mat_U_list, self.grad_x_mat_VT_list]

        self.grad_y_mat_U_list = tf.stack(self.grad_y_U_mat_list, axis=0)
        self.grad_y_mat_VT_list = tf.stack(self.grad_y_VT_mat_list, axis=0)

        self.grad_y_mat_svd_list = [self.grad_y_mat_U_list, self.grad_y_mat_VT_list]
