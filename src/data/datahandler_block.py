...
class DataHandler2D():
    ...
    def __init__(self, fespace, domain, num_blocks, rank_list, dtype):
        """
        ...
        self.dtype = dtype
        self.cells_per_block = fespace.n_cells // self.num_blocks
        ...
        for block_idx in range(num_blocks):
            start_cell = block_idx * self.cells_per_block
            end_cell = (block_idx + 1) * self.cells_per_block
            ...
            grad_x_mat_block = list(decomposition(grad_x_mat_block, rank_list))
            grad_y_mat_block = list(decomposition(grad_y_mat_block, rank_list))

            self.shape_val_mat_blocks.append(shape_val_mat_block)
            self.grad_x_mat_blocks.append(grad_x_mat_block)
            self.grad_y_mat_blocks.append(grad_y_mat_block)
