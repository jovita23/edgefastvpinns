...
class FE2D_Cell_SVD:
    ...
    """
    def __init__(self, cell_coordinates: np.ndarray, cell_type: str, fe_order: int, fe_type: str, \
                 quad_order: int, quad_type: str, fe_transformation_type: str, forcing_function,
                 svd_rank: int):
        ...
        #check for svd rank
        if (self.svd_rank > min(self.fe_order**2, self.quad_order**2)):
            print("[ERROR]  Current Quad Dimension is ", self.quad_order**2 , " and FE Dimension is ", self.fe_order**2)
            print("[ERROR]  SVD Rank is greater than the minimum of the two. Please reduce the SVD Rank")
            raise ValueError("SVD Rank is greater than the minimum of the two. Please reduce the SVD Rank")
        ...
        self.fe_setup = FE2DSetupMain(cell_type=self.cell_type, fe_order=self.fe_order, fe_type=self.fe_type, quad_order=self.quad_order, quad_type=self.quad_type)
    ...
    def assign_fe_transformation(self) -> None:
        ...
        """
        self.fetransformation = self.fe_setup.assign_fe_transformation(self.fe_transformation, self.cell_coordinates)
        # Sets cell co-ordinates for the FE Transformation
        self.fetransformation.set_cell()
        
        # obtains the Jacobian of the transformation
        self.jacobian = self.fetransformation.get_jacobian(self.quad_xi, self.quad_eta).reshape(-1,1)
    ...
    def assign_basis_values_at_quadrature_points(self) -> None:
        ...
        grad_x_orig, grad_y_orig = self.fetransformation.get_orig_from_ref_derivative(grad_x_ref, grad_y_ref, self.quad_xi, self.quad_eta)
        ...
        # perform SVD on the basis functions
        U_basis, S_basis, V_basis = np.linalg.svd(self.basis_at_quad, full_matrices=False)
        U_gradx, S_gradx, V_gradx = np.linalg.svd(self.basis_gradx_at_quad, full_matrices=False)
        U_grady, S_grady, V_grady = np.linalg.svd(self.basis_grady_at_quad, full_matrices=False)

        # concatenate the matrices based on the svd rank
        self.U_basis_reduced = U_basis[:, :self.svd_rank]
        self.U_gradx_reduced = U_gradx[:, :self.svd_rank]
        self.U_grady_reduced = U_grady[:, :self.svd_rank]

        self.VT_basis_reduced = V_basis[:self.svd_rank, :] * S_basis[:self.svd_rank, None]
        self.VT_gradx_reduced = V_gradx[:self.svd_rank, :] * S_gradx[:self.svd_rank, None]
        self.VT_grady_reduced = V_grady[:self.svd_rank, :] * S_grady[:self.svd_rank, None]

        
        

        # Commented out by thivin -- To make it faster for the Edge implementation 
        # For Double derivatives, uncomment the below codes. 
        ...
        # grad_xx_orig, grad_xy_orig, grad_yy_orig = self.fetransformation.get_orig_from_ref_second_derivative(grad_xx_ref, grad_xy_ref, grad_yy_ref, self.quad_xi, self.quad_eta)
