...
class DenseModelEdge(tf.keras.Model):
    ...
    def __init__(self, layer_dims, learning_rate_dict, params_dict, loss_function, input_tensors_list, orig_factor_matrices , force_function_list, \
        ...
        self.layer_list.append(layers.Dense(30, input_shape=(2,), activation='tanh'))
        ### TTD Model Starts ###
        # self.layer_list.append(layers.Dense(30, input_shape=(2,), activation='tanh',
        #                                             kernel_initializer='glorot_uniform',
        #                                             dtype=self.tensor_dtype, bias_initializer='zeros'))
        # self.layer_list.append(t3f.nn.KerasDense(input_dims=[5,3,2,1], output_dims=[5,3,2,1],
        #                             tt_rank=2, activation='tanh',
        ...
        #######################################################################################
        # # ttd Layer
        tt_layer = t3f.nn.KerasDense(input_dims=[5,3,2,1], output_dims=[5,3,2,1],
                                    tt_rank=2, activation='relu',
                                    bias_initializer=1e-3)

        self.layer_list.append(tt_layer)

        tt_layer_2 = t3f.nn.KerasDense(input_dims=[3,2,5,1], output_dims=[3,2,5,1],
                                        tt_rank=2, activation='relu',
                                        bias_initializer=1e-3)

        self.layer_list.append(tt_layer_2)
        # # model.add(Dense(30, activation='tanh'))
        self.layer_list.append(layers.Dense(1))
