...
class DenseModel_Inverse(tf.keras.Model):
    def __init__(self, layer_dims, learning_rate_dict, params_dict, loss_function, input_tensors_list, orig_factor_matrices , force_function_list, \
        ...
        for key, value in self.inverse_params_dict.items():
            self.inverse_params_dict[key] = tf.Variable(value, dtype=self.tensor_dtype, trainable=True)
    ...
    def train_step(self, beta=10, bilinear_params_dict=None):
        ...
        with tf.GradientTape(persistent=True) as tape:
            ...
            cells_residual = self.loss_function(test_shape_val_mat = self.pre_multiplier_val, test_grad_x_mat = self.pre_multiplier_grad_x, \
                                ...
                                forcing_function = self.force_matrix, bilinear_params = bilinear_params_dict, inverse_params_dict = self.inverse_params_dict)
            ...
            # Sensor loss
            sensor_loss = tf.reduce_mean(tf.square(predicted_sensor_values - self.sensor_values), axis=0)
        ...
        return {"loss_pde": total_pde_loss, "loss_dirichlet": boundary_loss, "loss": total_loss, "inverse_params": self.inverse_params_dict, "sensor_loss": sensor_loss}
