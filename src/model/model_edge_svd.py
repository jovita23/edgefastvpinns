...
# Custom Model
class DenseModelEdgeSVD(tf.keras.Model):
    """
    ...
    def __init__(self, layer_dims, learning_rate_dict, params_dict, loss_function, input_tensors_list, orig_factor_matrices , force_function_list, \
        ...
        super(DenseModelEdgeSVD, self).__init__()
    ...
    def train_step(self, beta=10, bilinear_params_dict=None):
        ...
        with tf.GradientTape(persistent=True) as tape:
            ...
            pred_grad_x = tf.reshape(gradients[:, 0], [self.n_cells, -1])  # shape : (N_cells , N_quadrature_points)
            pred_grad_y = tf.reshape(gradients[:, 1], [self.n_cells, -1])  # shape : (N_cells , N_quadrature_points)
            
            pred_val = tf.reshape(predicted_values, [self.n_cells, -1])  # shape : (N_cells , N_quadrature_points)
