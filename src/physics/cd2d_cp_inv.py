# This file contains the PDE loss function for the 2D convection-diffusion equation after decomosing the gradient tensor using cp decomposition
#Author: Jovita
...
def pde_loss_cd2d(test_shape_val_mat, test_grad_x_mat, test_grad_y_mat, pred_nn, pred_grad_x_nn, pred_grad_y_nn, forcing_function, bilinear_params,  inverse_params_list):
    ...
    result_diff_x = tf.einsum('kr, ik -> ir', factors_diff_x[2],pred_grad_x_nn*diffusion_coeff_NN)
    ...
    result_diff_y = tf.einsum('kr, ik -> ir', factors_diff_y[2],pred_grad_y_nn*diffusion_coeff_NN)
