...
def rhs(x, y):
    ...
    #return ((-0.2*x*np.sin(x*y)*np.cos(x) - y*np.sin(x)*np.sin(x*y) + 1.96*np.cos(x)*np.cos(x*y))*np.exp(-0.2*y)*np.sin(x))
...
def get_boundary_function_dict():
    ...
    """
    return {1000: boundary_function, 1001: boundary_function}
