"""
The file hosts the Neural Network (NN) model and the training loop for variational Physics-Informed Neural Networks (PINNs).
...
class DenseModelEdge(tf.keras.Model):
    ...
    def __init__(
        ...
        # self.core_x = orig_factor_matrices[3]
        # self.core_y = orig_factor_matrices[5]
        # self.core   = orig_factor_matrices[7]



        # self.factors_x = orig_factor_matrices[4]
        # self.factors_y = orig_factor_matrices[6]
        # self.factors   = orig_factor_matrices[8]
   ...
   #     )
    ...
    #             test_shape_val_mat=[self.core,self.factors],
    #             test_grad_x_mat=[self.core_x,self.factors_x],
    #             test_grad_y_mat=[self.core_y,self.factors_y],
    #             pred_nn=pred_val,
    ...
    def train_step(self, num_blocks=None, beta=10, bilinear_params_dict=None):
        ...
        with tf.GradientTape(persistent=True) as tape:
            ...
            # Dynamically determine the number of cells in each block
                ...
                # Split the gradients into x and y components and reshape them to (-1, 1)
                # the reshaping is done for the tensorial operations purposes (refer Notebook)
                block_pred_grad_x = tf.reshape(gradients[:, 0], [num_cells_in_block, pre_multiplier_val.shape[-1]])  # shape : (N_cells , N_quadrature_points)
                block_pred_grad_y = tf.reshape(gradients[:, 1], [num_cells_in_block, pre_multiplier_val.shape[-1]])  # shape : (N_cells , N_quadrature_points)    
                block_pred_val = tf.reshape(block_predicted_values, [num_cells_in_block, pre_multiplier_val.shape[-1]]) 
